...
**CLI commands:** `cast <intent>`, `history`, `choose <N>`.
**PWA views:** *cast* and *history* (a tab bar; no `choose` equivalent).
...
- CLI: `magick/cli.py` (Typer app + commands), `magick/cast.py` (message + logging),
  `magick/choose.py` (Todoist fetch + random sample). Entry point `magick` →
  `magick.cli:main`. `choose` needs `TODOIST_API_TOKEN` in the env.
